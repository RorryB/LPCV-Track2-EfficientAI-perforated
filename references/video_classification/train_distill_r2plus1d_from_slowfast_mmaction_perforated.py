...
CUDA_VISIBLE_DEVICES=1 ./ENV/bin/python -m pdb ./references/video_classification/train_distill_r2plus1d_from_slowfast_mmaction.py --data-path ./Dataset/QEVD_sup_full --teacher-config ./mmaction/mmaction2/configs/recognition/slowfast/slowfast_r101_16x4_QEVD_sup.py --teacher-checkpoint ./models/slowfast_r101_16x4_QEVD_sup.pth --student-weights KINETICS400_V1 --clip-len 16 --student-clip-len 16 --teacher-train-resize-size 256 256 --teacher-train-crop-size 224 224 --student-train-resize-size 128 171 --student-train-crop-size 112 112 --student-val-resize-size 128 171 --student-val-crop-size 112 112 --distill-alpha 0.5 --distill-temp 2.0 --cache-dataset --epochs 30 --batch-size 32 --lr 0.005 --output-dir ./checkpoint --amp --print-freq 1000 --pai-load-folder r2plus1d_dendritic
...
def build_student_model(student_model_name, num_classes, weights_name):
    supported_models = {"r2plus1d_18", "r3d_18", "mc3_18", "s3d"}
    ...
    if student_model_name == "s3d":
        in_ch = model.classifier[1].in_channels
        model.classifier[1] = nn.Conv3d(in_ch, num_classes, kernel_size=(1, 1, 1))
    else:
...
def main(args):
    ...
    # PAI Configuration for 3D video CNN (r2plus1d_18)
    GPA.pc.set_testing_dendrite_capacity(False)  # Debugging flag - start with True
    GPA.pc.set_module_names_to_perforate(["Linear", "Conv3d"])
    GPA.pc.set_module_ids_to_track([".stem", ".layer1", ".layer2", ".layer3", ".layer4", ".features"])
    GPA.pc.set_output_dimensions([-1, 0, -1, -1, -1])  # [batch, channels, time, height, width]
    GPA.pc.set_when_to_switch_mode("FIXED_SWITCH_MODE")
    GPA.pc.set_fixed_switch_num(30)
    GPA.pc.set_first_fixed_switch_num(30)
    ...
    student = UPA.perforate_model(student, save_name=f"{args.student_model}_dendritic", maximizing_score=True)
    if args.pai_load_folder:
        student = UPA.load_system(student, args.pai_load_folder, "latest", True)
    student.to(device)
    ...
    while True:
        ...
        student, restructured, training_complete = GPA.pai_tracker.add_validation_score(eval_metrics["video_acc1"], student)
        student.to(device)
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Load PerforatedAI system state from this folder using checkpoint name 'latest'.",
    )
    ...
    parser.add_argument("--student-model", default="r2plus1d_18", choices=["r2plus1d_18", "r3d_18", "mc3_18", "s3d"])
