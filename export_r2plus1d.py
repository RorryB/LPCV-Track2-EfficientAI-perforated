...
def export_to_onnx(model: torch.nn.Module, output_path: Path):
    h, w = input_shape
    dummy = torch.randn(1, 3, 16, 112, 112, dtype=torch.float32)
    torch.onnx.export(
        model,
        dummy,
        str(output_path),
...
def export_model(
    ...
    **additional_model_kwargs
    ...
    if checkpoint_path is not None:
        ...
        print("No checkpoint_path provided. Using pretrained/base model weights.")
    
    model.input_mean = torch.Tensor([0., 0., 0.]).reshape(1, 3, 1, 1, 1)
    model.input_std = torch.Tensor([1., 1., 1.]).reshape(1, 3, 1, 1, 1)    
    ...
    downloaded_model_path = output_dir
    if not skip_downloading and tool_versions is not None:
        print(f"Downloading model asset for {model_name} compiled for {target_runtime}.")
